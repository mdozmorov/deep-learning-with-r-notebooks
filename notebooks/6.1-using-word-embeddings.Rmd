...
# Each epoch takes ~3sec, total under 1min
history <- model %>% fit(
  x_train, y_train,
  epochs = 10,
  batch_size = 32,
...
imdb_dir <- "data/aclImdb" # See Data/Teaching/ML/aclImdb_v1.tar.gz
...
glove_dir = "data/glove.6B" # See Data/Teaching/ML/glove.6B.zip
...
# Each epoch takes ~1sec, total time is under 1min
history <- model %>% fit(
  x_train, y_train,
  epochs = 20,
  batch_size = 32,
...
save_model_weights_hdf5(model, "data/6_1_pre_trained_glove_model.h5")
```
...
# Each epoch is ~1sec, total under 1min
history <- model %>% fit(
  x_train, y_train,
  epochs = 20,
  batch_size = 32,
...
```{r}
model %>% 
  load_model_weights_hdf5("data/6_1_pre_trained_glove_model.h5") %>% 
